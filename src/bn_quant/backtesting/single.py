...
from bn_quant.config import RiskConfig
from bn_quant.domain import BacktestReport, Candle, EquityPoint, FundingRate, MarketEvidence, Position, Trade
from bn_quant.domain.evidence import taker_buy_ratio_from_candle
from bn_quant.execution import ExecutionEngine, entry_side_allowed, standalone_position_size
...
class Backtester:
    def __init__(self, strategy: TradingStrategy, risk: RiskConfig) -> None:
        ...
        self.risk = risk
        self.execution = ExecutionEngine(risk)
    ...
    *,
    ...
    for idx in range(required_history, len(fast_candles) - 1):
        ...
        if position is None:
            ...
            funding_idx, funding_delta = execution.apply_funding_until(
                position,
                funding_events,
                funding_idx,
                candle.close_time,
...
def execution_price_for_entry(execution: ExecutionEngine, price: float, side: int) -> float:
    from bn_quant.execution import apply_entry_slippage
...
def execution_stop_price(config, signal, entry_price):
    from bn_quant.execution import signal_stop_price
