...
from .market import Candle, MarketEvidence
...
def _open_interest_change_pct(rows: list[dict[str, Any]] | None) -> float | None:
    ...
    first = _optional_float(ordered[0].get("sumOpenInterest"))
    last = _optional_float(ordered[-1].get("sumOpenInterest"))
...
def _taker_buy_ratio_from_rows(rows: list[dict[str, Any]] | None) -> float | None:
    if not rows:
        return None
    latest = sorted(rows, key=lambda item: int(item.get("timestamp", 0)))[-1]
    buy = _optional_float(latest.get("buyVol")) or 0.0
    sell = _optional_float(latest.get("sellVol")) or 0.0
    total = buy + sell
    if total <= 0:
        return None
