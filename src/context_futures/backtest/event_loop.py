...
from .events import DataAvailableEvent
...
def run_event_loop(
    *,
    ...
    events = sorted(
        {
            DataAvailableEvent(
                time=candle_available_at(candle),
                run_idx=run_idx,
    ...
    equity_curve: list[EquityPoint] = [EquityPoint(events[0].time, state.equity(marks))]
    total_funding = 0.0

    for event in events:
        run = runs[event.run_idx]
        candle = run.fast[event.candle_idx]
        next_candle = run.fast[event.candle_idx + 1]
        ...
        if key in state.positions:
            _handle_existing_position(execution, fill_policy, state, run, event.candle_idx, key, marks)
        if key not in state.positions and _within_time_window(next_candle.open_time, start_time, end_time):
            _maybe_open_position(execution, state, risk_manager, run, event.candle_idx, marks)
...
def _handle_existing_position(
    ...
    if opposite is not None:
        from context_futures.execution import apply_exit_slippage
...
def _maybe_open_position(
    ...
    from context_futures.execution import apply_entry_slippage, signal_stop_price
